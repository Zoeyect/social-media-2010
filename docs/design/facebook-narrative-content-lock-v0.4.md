...
All late-evening seed stories visible at the canonical `2010-10-20 00:02 PT` session boundary are locked to October 19 PDT with explicit ISO timestamps. Future-dated seed records fail validation; the formatter warns in DEV and uses absolute metadata rather than disguising negative age as `just now`.

Feed relative time is additionally calendar-day scoped in `America/Los_Angeles`. The current runtime remains on October 20 from 00:02 through 00:17. October 19 history retains its date and absolute metadata; eligible same-day live stories may use relative wording. Detail views preserve each record’s own date and time.
