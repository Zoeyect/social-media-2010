...
`IG05` and `IG06` are design/narrative labels found in earlier planning language;
they are not runtime identities. The actual replacement runtime/media identity is
`june-ig-01`. This document does not rename that implementation identity or
invent additional runtime IDs.
