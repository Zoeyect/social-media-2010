...
| Facebook | 2010 Home launcher; people search; Feed Status/Check In controls; Friends/Pages/Requests; shared Events/RSVP; Places; registry-backed Photos; conservative Chat; derived Notifications | v0.3 implemented and code-tested | Groups is `REJECTED FOR TARGET DATE`; row 2 position remains empty; no user-projection violations introduced | Exact raster icons, Photo upload, complete Chat, and C-level pixel fidelity remain HOLD; manual browser sweep required | `Facebook 2010 Home Launcher & Native Feature Expansion v0.3` |
