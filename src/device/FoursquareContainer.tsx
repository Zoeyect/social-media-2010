...
import { getFoursquareVenueTodo, sortFoursquareTodos, type FoursquareTodoItem, type FoursquareVenueTodoItem } from "../data/foursquareTodos";
  ...
  return <section className="foursquare-container" aria-label="Foursquare" data-chrome-status="RECONSTRUCTED_FROM_PERIOD_SCREENSHOT">
    ...
    <main className="foursquare-content">
      {state.currentView === "root" && <div ref={rootRef} className={`foursquare-root is-${state.activeTab}`} onScroll={event => dispatch({ type: "SET_ROOT_SCROLL_POSITION", tab: state.activeTab, scrollPosition: event.currentTarget.scrollTop })}>
        ...
        {state.activeTab === "todos" && <TodosRoot todos={state.todos} venues={state.venues} onOpenVenue={venueId => dispatch({ type: "OPEN_VENUE", venueId, scrollPosition: state.rootScrollPositions.places })} />}
...
function TodosRoot({ todos, venues, onOpenVenue }: { todos: readonly FoursquareTodoItem[]; venues: readonly FoursquareVenue[]; onOpenVenue: (venueId: string) => void }) {
  const venueTodos = sortFoursquareTodos(todos).filter((todo): todo is FoursquareVenueTodoItem => todo.kind === "venue");
  return <section className="foursquare-todos-root" aria-label="To-Dos" data-content-status="RECONSTRUCTED_FROM_EXISTING_PROJECT_MATERIAL">{venueTodos.map(todo => {
    const venue = venues.find(candidate => candidate.id === todo.venueId);
    if (!venue) return null;
    return <button key={todo.id} type="button" className="foursquare-todo-venue-row" onClick={() => onOpenVenue(venue.id)}><strong>{venue.name}</strong></button>;
  })}</section>;
}
