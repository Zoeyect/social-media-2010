...
const persistence = {
  ...
  discardPersistedCameraPhoto: async () => {}, persistCameraCapturedArtifact: async artifact => (persistedOwners.push(artifact.snapshot.experienceSessionId), { ...artifact.snapshot, blob: artifact.blob, id: `capture-${++captureSequence}`, filename: `IMG_${captureSequence}.JPG`, captureSequence, origin: "player-camera" }),
...
function projectSoftware(tree) {
  ...
  const home=nodes.find(node=>node.props?.['aria-label']==='Home button');
  ...
  return {screen, powerControl: {begin:()=>power.props.onPointerDown(),end:()=>power.props.onPointerUp(),state:screen?.props.display.session.phase==='sleeping'?'asleep':'awake'},
    onHomePress:()=>{home.props.onPointerDown({button:0,pointerId:1,currentTarget:{setPointerCapture(){}}});home.props.onPointerUp({pointerId:1,currentTarget:{releasePointerCapture(){}}});},
    ...
    sessionDiagnostics:{experienceSessionId:screen?.props.display.session.experienceSessionId,sessionStartedAt:screen?.props.display.session.sessionStartEpochMs,
      cameraSceneSessionId:nodes.find(node=>node.type?.name==='MediaAttachmentDebug')?.props.cameraSceneSessionId,elapsedMs:screen?.props.display.elapsed,softwarePhase:screen?.props.display.session.phase},
  };
}
try {
  ...
  const open = async app => {
    if(view.sessionDiagnostics.softwarePhase==="app")await home();
  ...
  for(let run=0;run<2;run++) {
    ...
    const sessionId=view.sessionDiagnostics.experienceSessionId;
    ...
    await tick(900000-screen().display.elapsed);await tick(2000);
